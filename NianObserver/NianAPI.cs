...
namespace STKProject.NianObserver
{
    ...
    public class NianApi
    {
        ...
        public NianApi()
        {
            ...
            client.DefaultRequestHeaders.Host = "api.nian.so";
            client.DefaultRequestHeaders.Referrer = new Uri("http://api.nian.so/");
        ...
        public string HttpPost(string addr, string content)
        {
            ...
            var result = client.PostAsync(addr, new StringContent(content)).Result;
            return result.Content.ReadAsStringAsync().Result;
        }

        public string HttpPost(string addr, FormUrlEncodedContent content)
        {
        ...
        private string Request(string uri, bool isAuth, string[] additionalParams=null)
        {
            ...
            if (additionalParams!=null && additionalParams.Length % 2 != 0)
                throw new ArgumentException("Invalid Argument additionalParams");
            StringBuilder sb = new StringBuilder();
            for (int i = 0;;)
            {
                if (additionalParams == null)
                    break;
                if (i >= additionalParams.Length)
                    break;
                if (i != 0)
                    sb.Append("&");
                sb.Append($"{additionalParams[i]}={additionalParams[i+1]}");
                i += 2;
            }
            ...
            for (int j = 0; j < 5; j++)
            {
                string ret = HttpGet(uriBuilder.ToString());
        ...
        public bool Login(string mailAddr, string passWord)
        {
            ...
            }
            {
                ...
                FormUrlEncodedContent content = new FormUrlEncodedContent(
                    new []
                    {
                        new KeyValuePair<string, string>("email",mailAddr),
                        new KeyValuePair<string, string>("password",byte2String)
                    });
                string ret = HttpPost(uriBuilder.ToString(), content);
                var jsonDoc = JObject.Parse(ret);
                if (jsonDoc["status"].Value<string>() == "200")
                {
